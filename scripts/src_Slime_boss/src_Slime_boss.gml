...
function Attack1(){
	for(var i = -72; i <=72; i+=72)
		for(var j = -72; j <=72; j+=72)
			if place_meeting(x+i,y+j,obj_Boss) and obj_Player.receiveDamage == true
			{ 
				obj_Player.receiveDamage = false
				actualLife-=10;
			}

if obj_Player.receiveDamage != false {image_alpha=1 image_blend = c_white} //cor normal quando não recebe dano
...
function Attack2(){
	if place_meeting(x,y,obj_Attack) and obj_Player.receiveDamage == true
	{ 
		obj_Player.receiveDamage = false
